...
import { Settings, Plus, Users, Edit, Trash2, LogOut, UserPenIcon ,User2} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const fetchConfigs = async () => {
    ...
    setConfigs(res.data);
    const initialInputs = {};
    res.data.forEach((c) => (initialInputs[c.key] = c.value));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br font-sans from-white via-pink-50 to-purple-100 flex flex-col">
      <header className="flex justify-between items-center px-6 py-4 shadow bg-white">
         ...
         <button onClick={() => localStorage.clear() || (window.location.href = '/frontman')} className="text-sm px-4 py-2 border border-gray-300 text-gray-800 rounded-lg hover:bg-red-100 transition">
            Logout
          </button>
      </header>

      <div className="flex justify-center my-6 space-x-4">
        <button onClick={() => setView('settings')} className={`px-6 py-2 rounded-full shadow text-sm font-semibold transition-all border ${view === 'settings' ? 'bg-purple-600 text-white' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}>
          <Settings className="inline-block mr-2" size={16} /> Settings
        </button>
        <button onClick={() => setView('users')} className={`px-6 py-2 rounded-full shadow text-sm font-semibold transition-all border ${view === 'users' ? 'bg-pink-500 text-white' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}>
  ...
  <div className="backdrop-blur-xl bg-white/60 border border-purple-200 rounded-2xl shadow-xl p-6">
    <div className="flex items-center justify-between mb-6 border-b border-purple-200 pb-3">
      ...
      <button onClick={() => setIsEditingConfig(!isEditingConfig)} className="text-sm text-purple-600 hover:underline flex items-center gap-1">
    ...
    <div className="space-y-5">
      {CONFIG_FIELDS.filter(f => f.key !== 'genai_enabled').map((field) => (
  ...
  <div className="backdrop-blur-xl bg-white/60 border border-pink-200 rounded-2xl shadow-xl p-6">
    ...
    <ul className="space-y-3">
      {prompts.map((p) => (
        <li key={p.id} className="bg-white/70 backdrop-blur px-4 py-3 rounded-xl shadow border border-pink-100 font-sans">
